...
import org.sbuild.plugins.javac.internal.ForkSupport

/**
 * Java Compiler Task.
 *
 * Use [[JavacTask$#apply]] to configure and execute it in one go.
 *
 */
object JavacTask {
   ...
   *
   * For parameter documentation see the [[JavacTask]] constructor.
   *
   * @since 0.4.0
   */
 ...
 * To actually start the compilation use [[JavacTask#execute]].
 *
 * To easily configure and execute the compiler in one go, see [[JavacTask$#apply]].
 *
 * @constructor
 * Creates a new Javac Compiler task instance.
...
class JavacTask(
  ...
  protected def compileInternal(args: Array[String]): Int = {
    ...
    val compilerClassLoader = compilerClasspath match {
      case Seq() =>
        classOf[JavacTask].getClassLoader
      case cp =>
        val cl = new URLClassLoader(cp.map { f => f.toURI().toURL() }.toArray, classOf[JavacTask].getClassLoader)
